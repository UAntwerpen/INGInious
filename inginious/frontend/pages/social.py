...
class AuthenticationPage(INGIniousPage):
    ...
    def GET(self, auth_id):
        if session.is_lti:
            return redirect(current_app.get_path("auth/signin/" + auth_id))
...
class CallbackPage(INGIniousPage):
    def process_callback(self, auth_id):
        ...
        if not user:
            return redirect(current_app.get_path("signin?callbackerror"))
        if not self.user_manager.bind_user(auth_id, user):
            return redirect(current_app.get_path("signin?binderror"))
